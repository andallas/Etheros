...
// Initialize
	this.init = function()
	{
		...
		// Movement & size attributes
		this.position = new Vec2({x: options.x || 0, y: options.y || 0});
		this.velocity = new Vec2({x: options.vel_x || 0, y: options.vel_y || 0});
...
// Update
	...
	{
		// Move player
		this.position.add(this.velocity.multScalar(Firestorm.clock.getDelta()));
		for(var i = 0; i < this.components.length; i++)
		{
			this.components[i].update(this.position);
		}

		// Apply friction
		this.velocity.multScalar(Firestorm.physics.friction());
...
// Functions
	this.accelerate = function(value)
	{
		...
		var force = this.velocity.add(this.direction.multScal(value * (acceleration / (mass / 10))));
	}
